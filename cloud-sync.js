...
(function () {
  ...
  /* ==========================================================
     PENGATURAN > NOTIFIKASI — "Pemberitahuan Promosi". Beda dgn
     kv_store/workspace_members di atas, tabel `promo_notifications`
     ini SATU DAFTAR GLOBAL (broadcast dari admin panel ke SEMUA akun
     yang login, bukan per-owner) -- lihat catatan RLS-nya di
     supabase-schema.sql/migrasi-promo-notifications.sql. Dipakai oleh
     initPemberitahuanPromosiPage() di script.js. Kalau belum login,
     kembalikan daftar kosong apa adanya (fitur cloud ini memang perlu
     akun, sama seperti kv_store) -- BUKAN error, supaya halamannya
     cukup menampilkan pesan "masuk dulu" tanpa perlu bedakan alasan
     gagal. ---- */
  window.cloudFetchPromoNotifications = async function () {
    ...
    const { data, error } = await sb.from('promo_notifications')
      ...
      .limit(50);
    if (error) return { ok: false, loggedIn: true, reason: 'db', error: error, data: [] };
