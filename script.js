...
const FAST_MENU_ORDER_DEFAULT = ['addtx', 'tagihan', 'laporan', 'dompet', 'pendapatan', 'pengaturan', 'kalkulator', 'kurs', 'scanner', 'camera', 'unduhvideo', 'silentheroics', 'aitool'];
...
function videoDlResItemHtml({ format, label, sub, downloadUrl, icon, badge, filetype }) {
  ...
  // Tag jenis file (MP4/MP3/JPG dst) -- ditaruh sbg span TERPISAH dari
  // sub-label ukuran file, BUKAN digabung 1 baris ("367.5 MB · MP4")
  // krn di kartu sekecil ini (1/3 lebar grid) gabungan begitu gampang
  // kepanjangan/wrap berantakan. Elemen sendiri + CSS sendiri (lihat
  // .videodl-res-item-type) bisa diatur ukuran/posisinya independen
  // dari sub-label tanpa bikin sub-label ikut kepanjangan.
  const filetypeHtml = filetype ? `<span class="videodl-res-item-type">${filetype}</span>` : '';
  if (downloadUrl) {
...
/* ==========================================================
   SILENT HEROICS (#silentHeroicsOverlay) -- pintasan baru di kartu
   Fast Menu Beranda (#fmHomeSilentHeroicsBtn) & grid "Menu Utama"
   halaman Fast Menu (#fmGridSilentHeroicsBtn). Buka/tutup pola SAMA
   PERSIS dgn openKalkulatorOverlay/openVideoDlOverlay di atas.

   Generator ide + hook + prompt image-to-video utk tema "Silent
   Heroics" (pahlawan tanpa nama yg menyelamatkan orang lain lalu
   pergi tanpa mengharap pujian), pakai Google Gemini API langsung dari
   browser user (tanpa Worker/backend sendiri, krn Gemini sudah
   mendukung CORS). API key TIDAK ditanam bawaan aplikasi (beda dgn
   Unduh Video) -- user WAJIB isi key Gemini miliknya sendiri (gratis
   dari Google AI Studio) lewat modal Pengaturan (#silentHeroicsSettingsModalOverlay,
   tombol gir di header), boleh 1 key atau upload file berisi banyak
   key yg bergilir otomatis kalau salah satu kena limit harian. Semua
   tersimpan di localStorage lewat cloudStorage, PERSIS pola
   #videoDlSettingsModalOverlay di atas. ---- */
...
function loadShSettings() {
  try {
    ...
    return { apiKey: raw.apiKey || '', keyPool: Array.isArray(raw.keyPool) ? raw.keyPool : [] };
  } catch (e) { return { apiKey: '', keyPool: [] }; }
}
function persistShSettings(data) {
  try { cloudStorage.setItem(SH_SETTINGS_KEY, JSON.stringify(data)); }
  catch (e) { /* biarkan gagal senyap */ }
}
const shState = { lang: 'id', count: 3, model: 'gemini-2.5-flash', keyIndex: 0, genCount: 0 };
...
document.getElementById('shBackBtn')?.addEventListener('click', closeSilentHeroicsOverlay);
document.getElementById('fmHomeSilentHeroicsBtn')?.addEventListener('click', openSilentHeroicsOverlay);
document.getElementById('fmGridSilentHeroicsBtn')?.addEventListener('click', () => {
  closeFastMenuOverlay();
  openSilentHeroicsOverlay();
});

/* ---- Modal Pengaturan API (#silentHeroicsSettingsModalOverlay) -- pola
   SAMA PERSIS dgn modal Pengaturan Unduh Video, ditambah upload file
   key pool (banyak key sekaligus, satu key per baris). ---- */
const shSettingsModal = document.getElementById('silentHeroicsSettingsModalOverlay');
...
function shRenderKeyPoolStatus() {
  ...
  if (shSettings.keyPool.length) {
    ...
    el.innerHTML = `<span>🔑 ${shSettings.keyPool.length} API key dimuat dari file — bergilir otomatis</span><button type="button" id="shClearKeyPoolBtn">Hapus</button>`;
    document.getElementById('shClearKeyPoolBtn')?.addEventListener('click', () => {
...
document.getElementById('shSettingsBtn')?.addEventListener('click', openSilentHeroicsSettingsModal);
document.getElementById('shSettingsCloseBtn')?.addEventListener('click', () => closeModal(shSettingsModal));
shSettingsModal?.addEventListener('click', (e) => { if (e.target === shSettingsModal) closeModal(shSettingsModal); });
shSettingsForm?.addEventListener('submit', (e) => {
  e.preventDefault();
  shSettings = { ...shSettings, apiKey: (shApiKeyInput.value || '').trim() };
  persistShSettings(shSettings);
...
document.getElementById('shKeyFileInput')?.addEventListener('change', async (e) => {
  ...
  try {
    ...
    persistShSettings(shSettings);
    shRenderKeyPoolStatus();
    showToast(`${unique.length} API key dimuat dari file.`);
  } catch (err) {
    showToast('Gagal membaca file. Pastikan formatnya .txt atau .csv biasa.', 'err');
  }
});

/* ---- Segmented control (jumlah ide & bahasa) + toggle + slider durasi ---- */
...
shWireSeg('shLangSeg', (v) => { shState.lang = v; });
shWireSeg('shCountSeg', (v) => { shState.count = parseInt(v, 10); });
...
shDurationInput?.addEventListener('input', () => { if (shDurVal) shDurVal.textContent = shDurationInput.value; });
...
/* ---- Susun prompt utk Gemini -- definisi tema & instruksi format
   dipertahankan sama persis dgn prototipe awal, cuma sumber input
   (skenario/durasi/jumlah/bahasa/toggle) diarahkan ke elemen halaman
   penuh di app ini. ---- */
function shBuildPrompt() {
  const duration = shDurationInput?.value || '15';
  const scenario = (document.getElementById('shScenarioInput')?.value || '').trim();
...
Silent Heroics adalah cerita tentang seseorang biasa (bukan petugas resmi, bukan pahlawan berseragam) yang secara TIDAK SENGAJA berada di lokasi yang tepat dan menyelamatkan orang lain dari kecelakaan fatal atau bahaya besar — lalu pergi begitu saja tanpa mengharapkan pujian, sering kali baru diketahui identitasnya lewat rekaman CCTV atau saksi mata setelah kejadian. Nuansa cerita: senyap, tak terduga, mengharukan, twist emosional di akhir (baru terungkap siapa penolongnya / apa yang nyaris terjadi).

TUGAS:
Buat ${shState.count} ide konten video berbeda untuk tema ini${scenario ? `, dengan elemen wajib berikut yang harus dimasukkan ke SEMUA ide: "${scenario}"` : ''}. Setiap ide harus punya premis kecelakaan/bahaya yang BERBEDA (misal: nyaris tertabrak kereta, nyaris tenggelam, nyaris kebakaran, benda jatuh, dsb) — jangan mengulang premis yang sama antar ide.

Untuk setiap ide, berikan:
1. "concept" — ringkasan cerita 2-3 kalimat dalam ${langLabel}, jelas menunjukkan siapa penyelamat, siapa korban, bahaya apa, dan twist di akhir.
2. "hook" — satu kalimat pembuka (untuk 3 detik pertama video) dalam ${langLabel} yang dirancang memaksimalkan retention & viral di algoritma Facebook, gunakan gaya curiosity-gap atau emotional-pull tanpa clickbait bohong.
3. "video_prompt" — prompt detail untuk AI image-to-video (durasi target ${duration} detik), ${forceEn ? 'WAJIB ditulis dalam BAHASA INGGRIS' : `ditulis dalam ${langLabel}`}. Prompt harus sinematik dan mencakup: deskripsi adegan & setting, gerakan kamera (cth: slow push-in, handheld tracking, static wide), pencahayaan & mood, ekspresi/emosi karakter, dan breakdown beat waktu yang membagi ${duration} detik menjadi 3-5 segmen berurutan (format "0-3s: ...", "3-8s: ...", dst) supaya bisa langsung dipakai generate dari gambar diam ke video. Jangan sertakan dialog/teks di layar — tema ini silent, andalkan visual & musik.
4. "title" — judul video yang sangat berpotensi viral, ${langLabel}, maksimal 90 karakter, gunakan gaya storytelling/curiosity tanpa terasa clickbait murahan.
5. "hashtags" — array 10-14 hashtag campuran: hashtag besar/umum (cth #fyp, #viral) dan hashtag niche relevan tema silent heroics, tanpa spasi, dalam format #tag.

Balas HANYA dalam format JSON sesuai schema yang diberikan, tanpa teks tambahan apa pun.`;
}
...
async function shCallGemini(apiKey, promptText) {
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${shState.model}:generateContent?key=${encodeURIComponent(apiKey)}`;
  const body = {
    contents: [{ role: 'user', parts: [{ text: promptText }] }],
    generationConfig: { temperature: 1.05, responseMimeType: 'application/json', responseSchema: SH_SCHEMA },
  };
  const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  if (!res.ok) {
    let detail = '';
    try { const j = await res.json(); detail = (j && j.error && j.error.message) || ''; } catch (e) { /* abaikan */ }
    throw new Error(`Gemini API error (${res.status}). ${detail}`);
  }
  const data = await res.json();
  const text = (data && data.candidates && data.candidates[0] && data.candidates[0].content && data.candidates[0].content.parts || [])
    .map((p) => p.text || '').join('');
  if (!text) throw new Error('Respons kosong dari Gemini. Coba lagi.');
  let parsed;
  try { parsed = JSON.parse(text); }
  catch (e) { throw new Error('Gagal parsing hasil JSON dari model.'); }
  if (!parsed.ideas || !Array.isArray(parsed.ideas)) throw new Error('Format hasil tidak sesuai schema.');
  return parsed.ideas;
}

async function shCallGeminiWithRotation(keys, promptText) {
  let lastErr;
  const startIdx = shState.keyIndex % keys.length;
  for (let i = 0; i < keys.length; i++) {
    const idx = (startIdx + i) % keys.length;
    const key = keys[idx];
    if (keys.length > 1) shShowStatus('loading', `Menghubungi Gemini dengan key ${idx + 1}/${keys.length}...`);
    try {
      const ideas = await shCallGemini(key, promptText);
      shState.keyIndex = (idx + 1) % keys.length;
      return ideas;
    } catch (err) {
      lastErr = err;
      const msg = (err.message || '').toLowerCase();
      const isQuotaOrAuthIssue = msg.includes('429') || msg.includes('quota') || msg.includes('resource_exhausted')
        || msg.includes('403') || msg.includes('permission') || msg.includes('api key not valid') || msg.includes('400');
      if (keys.length > 1 && isQuotaOrAuthIssue) {
        shShowStatus('loading', `Key ${idx + 1}/${keys.length} bermasalah (limit/invalid), mencoba key berikutnya...`);
        continue;
      }
      throw err;
    }
  }
  throw lastErr || new Error('Semua API key di pool gagal digunakan.');
}

function shCopyText(txt, btnEl) {
  if (!(navigator.clipboard && navigator.clipboard.writeText)) return;
  navigator.clipboard.writeText(txt).then(() => showToast('Disalin ke clipboard')).catch(() => showToast('Gagal menyalin', 'err'));
}
...
function shRenderIdeas(ideas) {
  ...
  ideas.forEach((idea) => {
    ...
    const hashtagsHtml = (idea.hashtags || []).map((h) => `<span>${escapeHtml(h)}</span>`).join('');
    ...
    card.innerHTML = `
      <div class="shp-result-head">
        <div>
          ...
          <div class="shp-result-title">${escapeHtml(idea.title || '')}</div>
        </div>
        <button type="button" class="shp-copy-all">Salin semua</button>
      </div>
      <div class="shp-block">
        <div class="shp-block-label"><span>Hook</span><span class="shp-copy" data-copy="hook">salin</span></div>
        <div class="shp-block-body shp-hook">${escapeHtml(idea.hook || '')}</div>
      </div>
      <div class="shp-block">
        <div class="shp-block-label"><span>Konsep</span></div>
        <div class="shp-block-body">${escapeHtml(idea.concept || '')}</div>
      </div>
      <div class="shp-block">
        <div class="shp-block-label"><span>Video prompt (image-to-video)</span><span class="shp-copy" data-copy="prompt">salin</span></div>
        <div class="shp-block-body shp-prompt">${escapeHtml(idea.video_prompt || '')}</div>
      </div>
      <div class="shp-block">
        <div class="shp-block-label"><span>Hashtag</span><span class="shp-copy" data-copy="tags">salin</span></div>
        <div class="shp-hashtags">${hashtagsHtml}</div>
      </div>
    `;

    card.querySelector('.shp-copy-all')?.addEventListener('click', (e) => shCopyText(allText, e.target));
    card.querySelector('[data-copy="hook"]')?.addEventListener('click', () => shCopyText(idea.hook || ''));
    card.querySelector('[data-copy="prompt"]')?.addEventListener('click', () => shCopyText(idea.video_prompt || ''));
    card.querySelector('[data-copy="tags"]')?.addEventListener('click', () => shCopyText((idea.hashtags || []).join(' ')));

    results.prepend(card);
  });
}

document.getElementById('shGenerateBtn')?.addEventListener('click', async () => {
  const keys = shGetActiveKeys();
  const btn = document.getElementById('shGenerateBtn');
  if (!keys.length) {
    shShowStatus('error', 'Atur dulu API key Gemini kamu lewat ikon gir di pojok kanan atas.');
    return;
  }
  btn.disabled = true;
  const originalLabel = btn.textContent;
  btn.textContent = 'Membuat ide...';
  shShowStatus('loading', `Menghubungi Gemini untuk membuat ${shState.count} ide baru...`);

  try {
    const promptText = shBuildPrompt();
    const ideas = await shCallGeminiWithRotation(keys, promptText);
    shRenderIdeas(ideas);
    shHideStatus();
  } catch (err) {
    shShowStatus('error', err.message || 'Terjadi kesalahan saat memanggil Gemini API.');
  } finally {
    btn.disabled = false;
    btn.textContent = originalLabel;
  }
...
let ccWatchId = null;
let ccActive = false;
let ccFacingMode = 'environment'; // 'environment' = kamera belakang (default), 'user' = kamera depan
let ccSwitchingCam = false;       // guard supaya tap tombol switch dobel2 tidak tabrakan getUserMedia
let ccLastFix = null;          // { lat, lng, accuracy, altitude, timestamp } -- bacaan GPS mentah paling baru
let ccAvgFix = null;           // { lat, lng, accuracy, altitude, timestamp } -- hasil rata-rata presisi tinggi (dipakai utk tampil/stempel)
...
// ---- Status muat peta mini utk jepretan yg lagi aktif (ccMapStatusKey
// dibandingkan ke ccMapKeyFor(ccCapturedFix), sama spt ccMapImgKey) --
// dipakai ccDrawMapThumbnail utk nampilin kartu "Memuat peta..." SELAGI
// request (atau retry-nya) masih jalan, lihat ccLoadMapForFix.
let ccMapStatus = null;      // null | 'loading' | 'failed'
let ccMapStatusKey = null;
let ccMapRetryCount = 0;
let ccMapRetryTimer = null;
let ccMapSpinnerTimer = null; // jadwal render-ulang berkala biar animasi spinner "Memuat peta..." jalan
const CC_MAP_MAX_RETRIES = 3;
const CC_MAP_RETRY_DELAY_MS = 1500; // jeda dasar antar-retry, dilipatgandakan tiap percobaan (backoff)
...
// lebih jauh dari CC_FIX_BUFFER_MAX_JUMP_M, buffer direset (dianggap
// lokasi baru) supaya rata-rata tidak "ketarik" ke posisi lama.
const CC_FIX_BUFFER_MAX_AGE_MS = 20000;
const CC_FIX_BUFFER_MAX_SIZE = 8;
const CC_FIX_BUFFER_MAX_JUMP_M = 60;

// Label kualitas akurasi GPS, dipakai di panel info & stempel foto --
// tidak pernah mengklaim "100% akurat" (mustahil dg GPS konsumer),
// hanya menandai seberapa presisi bacaan yg didapat saat ini.
...
// Gabungkan beberapa bacaan GPS terakhir (weighted by 1/akurasi²,
// makin akurat sebuah bacaan makin besar pengaruhnya) jadi satu titik
// yg lebih stabil dibanding satu bacaan tunggal -- teknik umum utk
// meredam jitter GPS di browser tanpa perlu sensor tambahan.
function ccUpdateFixBuffer(fix) {
  ...
  if (ccFixBuffer.length) {
    ...
    if (ccHaversineMeters(ref.lat, ref.lng, fix.lat, fix.lng) > CC_FIX_BUFFER_MAX_JUMP_M) {
      ccFixBuffer = []; // lompat jauh -> dianggap lokasi baru, mulai buffer dari nol
    }
  }
  ...
  let sumW = 0, sumLat = 0, sumLng = 0, sumAltW = 0, sumAlt = 0, minAcc = Infinity;
  for (const f of ccFixBuffer) {
    const acc = (typeof f.accuracy === 'number' && f.accuracy > 0) ? f.accuracy : 15;
    const w = 1 / (acc * acc);
    sumW += w; sumLat += f.lat * w; sumLng += f.lng * w;
    if (typeof f.altitude === 'number') { sumAltW += w; sumAlt += f.altitude * w; }
    if (acc < minAcc) minAcc = acc;
  }
  // Estimasi akurasi gabungan: makin banyak bacaan konsisten yg
  // dirata-rata, makin kecil (makin baik) dibanding bacaan tunggal
  // terbaik -- rumus kombinasi varians standar utk rata-rata tertimbang.
  const combinedAcc = sumW > 0 ? Math.max(1, Math.min(minAcc, 1 / Math.sqrt(sumW))) : minAcc;
  ccAvgFix = {
...
// Kategori POI yg dikenali Nominatim sbg "jenis tempat" -- kalau titik
// GPS jatuh persis di salah satu objek ini (toko, tempat ibadah,
// kantor, dst.), field address[kategori] berisi NAMA tempatnya (mis.
// address.shop = "Indomaret", address.amenity = "Masjid Al-Ikhlas").
// Diperiksa berurutan dari yg paling spesifik ke paling umum.
...
function ccExtractExactPOIName(data) {
  ...
  //    (bukan cuma di tengah jalan/area kosong).
  if (data.name && String(data.name).trim()) return String(data.name).trim();
  if (data.namedetails && data.namedetails.name) return String(data.namedetails.name).trim();
  // 2) Fallback: cek tag kategori pada objek yg ditemukan (data.category/
  //    data.type dari Nominatim menunjukkan jenis objeknya, mis.
  //    category:"shop", type:"supermarket") -- kalau ada dan bukan cuma
  //    penanda kategori jalan/wilayah administratif, & address-nya
  //    kebetulan punya key yg sama berisi nama, pakai itu.
  const addr = data.address || {};
  if (data.category && addr[data.category] && data.category !== 'place' && data.category !== 'highway' && data.category !== 'boundary') {
...
// display_name mentah dari Nominatim) -- urutan mengikuti konvensi
// alamat Indonesia: nama jalan+no rumah, dusun/kelurahan, kecamatan,
...
function ccBuildFullAddress(data) {
  ...
  const sub = a.suburb && a.suburb !== local ? a.suburb : (a.city_district || a.subdistrict);
...
function ccOnPosition(pos) {
  ...
  // Rata-ratakan dgn bacaan2 GPS terakhir (weighted by akurasi) supaya
  ...
  if (accEl) accEl.textContent = (typeof avg.accuracy === 'number') ? `± ${avg.accuracy.toFixed(1)} m (${qualityLabel})` : '–';
  if (altEl) altEl.textContent = (avg.altitude !== null && avg.altitude !== undefined) ? `${Math.round(avg.altitude)} m dpl` : 'Tidak tersedia';
  if (tsEl) tsEl.textContent = new Date(avg.timestamp).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'medium' });
  // "Terkunci" kalau akurasi gabungan sudah cukup baik (ikon GPS jadi
  // hijau); di atas itu tetap ditampilkan tapi ditandai masih
  // menyempurnakan sinyal. Ambang diperketat (15m, dari sebelumnya 30m)
  // karena sekarang nilainya sudah hasil rata-rata yg lebih stabil.
  const locked = typeof avg.accuracy === 'number' && avg.accuracy <= 15;
  ccSetGpsStatus(locked ? `GPS terkunci • ${qualityLabel} (± ${avg.accuracy.toFixed(1)} m)` : `Menyempurnakan sinyal • ${qualityLabel} (± ${(avg.accuracy || 0).toFixed(1)} m)`, locked ? 'locked' : 'searching');
  const actionsRow = document.getElementById('ccActionsRow');
  if (actionsRow) actionsRow.hidden = false;

  const now = Date.now();
  const movedEnough = ccLastGeocodeLat === null || ccHaversineMeters(ccLastGeocodeLat, ccLastGeocodeLng, avg.lat, avg.lng) >= CC_GEOCODE_MIN_DELTA_M;
  ...
  if (movedEnough || (ccLastGeocodeAt === 0)) {
    if (movedEnough && dueForRefresh || ccLastGeocodeAt === 0) {
      ccLastGeocodeAt = now; ccLastGeocodeLat = avg.lat; ccLastGeocodeLng = avg.lng;
...
function ccStopCamera() {
  ...
  if (ccMapRetryTimer) { clearTimeout(ccMapRetryTimer); ccMapRetryTimer = null; }
  if (ccMapSpinnerTimer) { clearTimeout(ccMapSpinnerTimer); ccMapSpinnerTimer = null; }
  ccMapStatus = null; ccMapStatusKey = null; ccMapRetryCount = 0;
}
...
// menyusul" di foto yg sama.
// Kalau request gagal (jaringan tidak stabil/CORS/kuota Geoapify habis),
// dicoba ULANG otomatis sampai CC_MAP_MAX_RETRIES kali dgn jeda yg makin
// panjang (backoff) -- SELAGI mencoba, ccMapStatus diset 'loading' supaya
// ccDrawMapThumbnail bisa nampilin kartu "Memuat peta..." di foto (baik
// percobaan pertama maupun tiap retry-nya). Baru kalau retry terakhir
// tetap gagal, ccMapStatus diset 'failed' & dibiarkan diam2 -- foto TETAP
// tersimpan normal, cuma tanpa peta.
function ccLoadMapForFix(fix, _retryCount) {
  ...
  // Percobaan pertama (bukan retry): reset status/hitungan retry &
  // langsung tandai 'loading' supaya indikator muncul SEBELUM request
  // network dimulai (termasuk saat baru jepret & foto pertama tampil
  // tanpa peta dulu).
  if (!isRetry) {
    if (ccMapRetryTimer) { clearTimeout(ccMapRetryTimer); ccMapRetryTimer = null; }
    ccMapRetryCount = 0;
    ccMapStatus = 'loading';
    ccMapStatusKey = key;
    if (ccMapKeyFor(ccCapturedFix) === key) ccRenderStampedPhoto(ccEditAddressText, ccEditPlaceText);
  }
  const url = ccBuildStaticMapUrl(fix.lat, fix.lng);
  if (!url) { ccMapStatus = 'failed'; return; }
  const img = new Image();
  ...
  img.onerror = () => {
    // Jepretan sudah berganti selagi request ini berjalan -- tidak perlu
    // retry lagi utk titik yg sudah tidak relevan.
    if (ccMapKeyFor(ccCapturedFix) !== key) return;
    if (retryCount < CC_MAP_MAX_RETRIES) {
      // Backoff: jeda makin panjang tiap percobaan (1.5s, 3s, 4.5s, ...)
      // supaya tidak langsung membombardir server pas jaringan lagi
      // goyang/putus-nyambung.
      ...
      // Retry terakhir tetap gagal -- menyerah diam2, kartu "Memuat
      // peta..." dilepas & foto tetap tersimpan normal tanpa peta.
      ccMapStatus = 'failed';
      ccMapStatusKey = null;
      if (ccMapKeyFor(ccCapturedFix) === key) ccRenderStampedPhoto(ccEditAddressText, ccEditPlaceText);
    }
...
function ccDrawMapThumbnail(ctx, w, h) {
  ...
  const loading = !ready && ccMapStatus === 'loading' && ccMapStatusKey === capturedKey;
  ...
  if (loading) {
    // Kartu placeholder "Memuat peta..." -- bentuk & posisi SAMA persis
    // dgn kartu peta asli (supaya tidak "loncat" begitu peta beneran
    // selesai dimuat & menggantikannya), isinya cuma teks + putaran
    // spinner sederhana biar kelihatan masih aktif proses (bukan macet),
    // penting terutama pas jaringan lambat/tidak stabil.
    ctx.save();
    ...
    // Spinner: busur putus-putus di tengah kartu, sudutnya digeser
    // berdasarkan waktu berjalan (Date.now()) supaya animasinya jalan
    // sendiri tiap kali stempel digambar ulang selama masih loading.
    const cx = x + size / 2, cy = y + size * 0.42, r = size * 0.15;
    ...
    ctx.stroke();
    ctx.restore();
    // Selagi masih loading, jadwalkan satu render ulang lagi supaya
    // spinner-nya terus berputar (bukan cuma diam di satu sudut) --
    // dibatalkan otomatis begitu status berubah (peta siap/gagal) krn
    // syarat `loading` di atas jadi false.
    if (!ccMapSpinnerTimer) {
      ccMapSpinnerTimer = setTimeout(() => {
        ccMapSpinnerTimer = null;
        if (ccMapStatus === 'loading' && ccMapStatusKey === ccMapKeyFor(ccCapturedFix)) {
          ccRenderStampedPhoto(ccEditAddressText, ccEditPlaceText);
        }
      }, 180);
    }
...
function ccRenderStampedPhoto(addrText, placeText) {
  ...
  const placeLines = placeName ? ccWrapCanvasText(ctx, placeName, w - pad * 2).slice(0, 1) : [];
...
async function ccCaptureFoto() {
  ...
  ccMapImg = null;
  ccMapImgKey = null;
  // Peta mini SELALU mulai dimuat dari nol persis saat jepret ditekan
  // (bukan prefetch selagi masih live-preview) -- foto pertama tampil
  // TANPA peta dulu (kartu "Memuat peta..." yg tampil, lihat
  // ccDrawMapThumbnail), lalu digambar ulang otomatis begitu peta
  // selesai dimuat (atau setelah beberapa kali retry, lihat
  // ccLoadMapForFix) supaya tetap tahan jaringan yg tidak stabil.
  ccLoadMapForFix(ccCapturedFix);
...
const NOTIF_DETAIL_ICONS = {
  ...
  hutang: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 6.7v10.6M9.3 15.3c0 1.15 1.1 1.9 2.7 1.9s2.7-.85 2.7-1.9-1.1-1.6-2.7-2-2.7-.85-2.7-1.9 1.1-1.9 2.7-1.9 2.7.75 2.7 1.9"/></svg>'
};
...
const aiPollinationsKeyInput = document.getElementById('aiPollinationsKeyInput');
...
if (aiSettingsTestBtn) aiSettingsTestBtn.addEventListener('click', async () => {
  ...
  if (!testWorkerUrl && !testKey) {
    ...
    aiTestResult.textContent = 'Isi API key dulu sebelum tes (atau isi URL Worker kalau pakai mode Worker).';
    return;
  }
  ...
  try {
    ...
    const okFlag = testWorkerUrl ? !!(resultJson && resultJson.ok !== false && res.ok) : res.ok;
    if (okFlag) {
      aiTestResult.style.color = '#059669';
      aiTestResult.textContent = 'Berhasil! Key & model ini valid dan bisa dipakai.';
    } else {
      const detail = resultJson ? (resultJson.detail || resultJson.error?.message || '') : '';
      aiTestResult.style.color = '#9F1239';
      aiTestResult.textContent = `Gagal (HTTP ${res.status}): ${detail || 'periksa kembali key/model/URL Worker.'}`;
    }
  } catch (e) {
    aiTestResult.style.color = '#9F1239';
    aiTestResult.textContent = 'Gagal menghubungi server AI: koneksi diblokir (cek internet/ad-blocker/VPN).';
  } finally {
...
async function sendAiMessage() {
  ...
  try {
    ...
    else if (err.status === 403) msg = `API key ${providerLabel} ditolak (403): ${err.message || 'periksa kembali key di Pengaturan, atau pastikan API sudah diaktifkan untuk key ini.'}`;
    else if (err.status === 404) msg = `Model "${activeModel}" tidak ditemukan (404) untuk API key ini. Coba ganti model di Pengaturan.`;
    else if (err.status === 429) msg = `Kuota/limit ${providerLabel} tercapai. Coba lagi nanti.`;
    else if (err.message === 'EMPTY_RESPONSE') msg = `${providerLabel} tidak memberi jawaban (mungkin diblokir filter keamanan). Coba ubah pertanyaan.`;
    else if (err.message) msg = `Gagal menghubungi ${providerLabel}: ${err.message}`;
...
// ---- Navigasi Tool AI: SATU baris 3 tab sejajar (Bantuan / Chat /
// Gambar), bukan lagi tab+toggle bertingkat (dulu: tab "Chat Umum"
// baru memunculkan toggle tersembunyi "Chat/Analisis" vs "Buat
// Gambar" -- 2 lapis navigasi yg gampang bikin user tidak sadar fitur
// "Buat Gambar" itu ada). Sekarang ketiganya langsung kelihatan &
// setara. "Chat" & "Gambar" tetap berbagi 1 riwayat percakapan
// (bucket 'kreatif') spt sebelumnya -- cuma beda apa yg dilakukan
// composer saat submit -- supaya konteks obrolan & histori lama user
// yang sudah tersimpan tidak hilang/pecah.
...
// Pollen mingguan akun tsb. Endpoint balas file gambar mentah
// (bukan JSON), jadi hasilnya dikonversi ke base64 di sini supaya
// bentuk return-nya tetap sama seperti sebelumnya ({ mimeType,
// base64 }) dan tidak perlu ubah kode pemanggilnya.
...
async function sendAtkMessage() {
  ...
  try {
    ...
    else if (err.status === 403) msg = `API key ${providerLabel} ditolak (403): ${err.message || 'pastikan API aktif untuk key ini.'}`;
    ...
    else if (err.status === 'NO_IMAGE') msg = `Gagal mengembalikan gambar: ${err.message}`;
    else if (err.message === 'EMPTY_RESPONSE') msg = `${providerLabel} tidak memberi jawaban (mungkin diblokir filter keamanan). Coba ubah pertanyaan.`;
    else if (err.message) msg = `Gagal menghubungi ${providerLabel}: ${err.message}`;
  ...
  if (!list.length) {
    ...
    empty.innerHTML = `
      ...
      <p>${atkActiveTab === 'bantuan'
        ...
        : atkCreativeMode === 'image'
          ? 'Jelaskan gambar yang ingin dibuat, ZayaDev akan menggambarnya.'
          : 'Tanya apa saja, atau lampirkan gambar/video (📎) untuk dianalisis.'}</p>
      <p class="ai-chat-empty-hint">${atkActiveTab === 'bantuan'
        ? 'Contoh: "Gimana cara ubah password?"'
        : atkCreativeMode === 'image'
          ? 'Contoh: "Kucing astronot gaya cat air"'
          : 'Contoh: "Ringkas isi gambar struk ini"'}</p>
    `;
